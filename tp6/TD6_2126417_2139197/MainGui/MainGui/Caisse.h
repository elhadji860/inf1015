...
public:
	Article() { prix = 0, taxable = 0; }
	Article(std::string desc, double prix_, bool tax) { description = desc; prix = prix_; taxable = tax; }
...
public:
	...
	void reinitialiser() 
	{ 
		for (auto x : articleAjoutes) {
			delete x;
		}
		articleAjoutes.clear();
	}
